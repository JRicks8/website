...
export class BroadPhaseSolver {
  ...
  solve(bodies) {
    for (const body of bodies) {
      body.collider.mesh.geometry.computeBoundingBox();
    }

    const res = [];
    for (let i = 0; i < bodies.length - 1; i++) {
      for (let j = bodies.length - 1; j > i; j--) {
        if (bodies[i].collider.mesh.geometry.boundingBox.intersectsBox(bodies[j].collider.mesh.geometry.boundingBox)) {
